...
| Config | `...4f4e46000000` | ESP32 → PWA (Read+Notify) | 16-byte packed PID+threshold |
...
| ID | Name | Payload |
|----|------|---------|
| `0x01` | SET_PID | `[cmdId: u8][Kp: f32][Ki: f32][Kd: f32][fpThr: f32]` (17 bytes total) |
| `0x02` | ARM | `[cmdId: u8][0\|1: u8]` (2 bytes) |
| `0x03` | FLASH_CFG | `[cmdId: u8]` (1 byte) — persists current live values to NVS |
| `0x04` | REBOOT | `[cmdId: u8]` (1 byte) — motor stops first, then restarts |
| `0x05` | CALIBRATE | `[cmdId: u8]` (1 byte) — auto-zeros pitch from current reading |
